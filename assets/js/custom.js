...
// Google Maps Rota Oluşturma Kodu
...
// Function to initialize the map after loading the API
function initMap() {
...
let routeShown = false; // Bir değişken ekleyin ve varsayılan olarak rota gösterilmedi olarak ayarlayın
...
function openModal() {
  ...
  // Show Route function
  function showRoute() {
	// Load the map only if it hasn't been loaded yet
	if (!map) {
	  loadMap();
	  return;
	}
...
function updateToOptions() {
  ...
  toSelect.querySelectorAll('option[value]:not([value=""])').forEach(option => option.remove());
...
fromSelect.addEventListener('change', updateToOptions);
 ...
 // Dönüş Tarihi Seçim Gizleme 
 const oneWayRadio = document.getElementById("oneWay");
const roundTripRadio = document.getElementById("roundTrip");
const returnDateInput = document.getElementById("roundTripDate");

// Gidiş-dönüş seçildiğinde dönüş tarihi seçimini göster
roundTripRadio.addEventListener("change", () => {
returnDateInput.style.display = "inline-block";
localStorage.setItem("tripType", "roundTrip");
});

// Gidiş seçildiğinde dönüş tarihi seçimini gizle
oneWayRadio.addEventListener("change", () => {
returnDateInput.style.display = "none";
localStorage.removeItem("tripType");
});

// Sayfa yüklendiğinde localStorage değerini kontrol et
